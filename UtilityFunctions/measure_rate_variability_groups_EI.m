function [Sig, Sig_zm, Sig_shuff, Sig_zm_shuff] = measure_rate_variability_groups_EI(rast,num_groups)
...
grpsize_e = 0.8*num_neurons/num_groups;
grpsize_i = 0.2*num_neurons/num_groups;
pvec = [kron(1:num_groups,ones(1,grpsize_e)) kron(1:num_groups,ones(1,grpsize_i))];
...
Zgr_zm = Zgrouped - diag(mean(Zgrouped,2))*ones(size(Zgrouped)); % same with zero mean

Sig = mean(std(Zgrouped)); % std deviation along columns (groups), mean along rows (over time)
Sig_zm = mean(std(Zgr_zm));

if nargout > 2
    num_rep = 10;
    Sig_shuff = zeros(1,num_rep);
    Sig_zm_shuff = zeros(1,num_rep);
    for i = 1:num_rep
        ...
        Sig_zm_shuff(i) = mean(std(Zgr_zm));
    end
    Sig_shuff = mean(Sig_shuff);
    Sig_zm_shuff = mean(Sig_zm_shuff);
end
